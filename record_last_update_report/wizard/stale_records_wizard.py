...
class StaleRecordsWizard(models.TransientModel):
    ...
    def name_get(self):
        # 14.0-16.0 compute display_name FROM name_get(); overriding only the
        # compute would leave name_get() answering "stale.records.wizard,4".
        return [(wizard.id, _('Stale Records Report')) for wizard in self]
...
class StaleRecordsLine(models.TransientModel):
    ...
    def name_get(self):
        return [(line.id, line.record_name or _('Record #%s') % line.res_id)
                for line in self]
