...
class ActivityOverdueReport(models.Model):
    ...
    construction: on this Odoo series a done or cancelled activity is deleted
    from mail_activity outright, so it cannot be reported. The module never
    writes to mail.activity or to any document.
    """
    ...
    res_id = fields.Integer(
        string='Document ID', readonly=True, group_operator=False,
    ...
    days_overdue = fields.Integer(
        string='Days Overdue', readonly=True, group_operator='avg',
    ...
    def _apply_ir_rules(self, query, mode='read'):
        ...
        Same idea as Odoo's own mail.activity._search, hooked one level lower:
        on this Odoo series read_group() builds its query through _where_calc()
        plus _apply_ir_rules() instead of _search(), so this is the single place
        that covers searching, counting, grouping and plain field reads alike.
        """
        ...
        visible_ids = self._visible_ids_from_rows(rows)
        if visible_ids:
            query.add_where('"%s"."id" IN %%s' % table, [tuple(visible_ids)])
        else:
