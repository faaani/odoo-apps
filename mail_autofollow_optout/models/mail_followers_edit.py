...
class MailFollowersEdit(models.TransientModel):
    ...
    def edit_followers(self):
        """Adding followers through the Edit Followers wizard is a deliberate
        human action, exactly like the Follow button: flag the whole call as
        a manual subscription so the auto-follow opt-out (which only targets
