...
class MailBounceReport(models.Model):
    ...
    has_email = fields.Boolean(
        ...
        help='Unticked when the contact carries no usable email address - '
             'either none at all, or something that cannot be parsed as one. '
             'Such a contact can still hold a bounce counter from an address '
             'that was later cleared, and can never match a blacklist entry.',
    )
    ...
    blacklist_date = fields.Datetime(
        ...
        help='When the blacklist entry row for this address was created. '
             'Removing an address from the blacklist archives the row rather '
             'than deleting it, so for an address that was removed and later '
             'blacklisted again this is the first time Odoo saw it.',
    )
