...
(res.partner.message_bounce), and its own Discuss code drops a contact from a
channel once that counter reaches 10 - but there is no screen anywhere that
shows who is bouncing. A mailing list therefore decays invisibly.
...
* an "Above Threshold" flag using an inclusive >= test against a threshold you
  configure in Settings (default 10, the limit Odoo's own Discuss code uses),
