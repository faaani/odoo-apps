...
Odoo already resolves a field *label* through ``ir.model.fields``:
``Field._description_string()`` reads ``ir.model.fields.field_description`` for
the current language and falls back to the value written in Python.
...
def _description_help(self, env):
    if env.lang and 'field.help.customization' in env:
        field_help = env['ir.model.fields'].get_field_help(self.base_field.model_name)
