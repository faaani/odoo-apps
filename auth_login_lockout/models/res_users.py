...
class ResUsers(models.Model):
    ...
    # ------------------------------------------------------------------
    # every helper runs in its own cursor: a failed login rolls the request
    # transaction back, which would otherwise take the evidence with it.
    #
    ...
    # ------------------------------------------------------------------
    def _lockout_guard(self, login, ip):
    ...
    def _lockout_record(self, login, ip, result):
    ...
    def _login(self, credential, user_agent_env):
        # 19.0 authenticates from a credential dict and returns an auth_info
        # dict instead of a uid. Only a credential that carries a login can be
        # throttled by login - a webauthn credential has none, so it is handed
        # straight to core.
        ...
        ip = _remote_ip()
        try:
            locked_until = self._lockout_guard(login, ip)
        except Exception:
        ...
        try:
            ...
            try:
                self._lockout_record(login, ip, 'failed')
            except Exception:
                _logger.exception('auth_login_lockout: could not record a failed login')
            raise

        try:
            self._lockout_record(login, ip, 'success')
        except Exception:
