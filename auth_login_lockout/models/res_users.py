...
class Users(models.Model):
    ...
    def _login(cls, db, credential, user_agent_env):
        # 18.0 authenticates from a credential dict. Only the password flow
        # carries a login string; ``type: 'webauthn'`` carries none at all, and
        # a login is what this module throttles - without one there is nothing
        # to count, so those flows are passed straight through untouched.
        ...
        try:
            # 18.0 returns an auth_info dict here, not a uid - it is handed back
            # to the caller exactly as received
            auth_info = super(Users, cls)._login(db, credential, user_agent_env=user_agent_env)
        except AccessDenied:
            try:
